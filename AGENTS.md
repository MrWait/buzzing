...
- 修改代码后无需自行编译验证，如果有编译报错，用户会显式提出问题。
- SDK、服务端业务逻辑中，需要添加足够日志，在请求出入口、关键处理节点、状态变更中。
- **锁规范（服务端）**：tokio::sync 的 Mutex/RwLock 不可重入且无死锁检测。
  - 禁止在持有某把锁时，调用会对**同一把锁**再次加锁的函数（如 `push_chat_update` 内部会对 chat 缓存重取读锁）。
  - 需要在持锁块内变更缓存状态、块外再调推送类函数时，先在锁内取出所需数据（如 `user_ids`、`version`），释放锁后再调用。
  - 读锁内可再取读锁（reader 可重入）；写锁内取读锁/写锁均会自死锁。
