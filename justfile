...
# 客户端在 android 平台启动
# 用法：just csa                      # 自动选择设备
#       just csa 5e02e4c9            # 指定设备 ID
[working-directory('buzzing')]
csa device_id="":
    flutter run {{ if device_id != "" { "-d " + device_id } else { "" } }}
...
init_data:
    set NODE_TLS_REJECT_UNAUTHORIZED=0 && node init.js
...
[working-directory('utils')]
init_data_test:
    set NODE_TLS_REJECT_UNAUTHORIZED=0 && node init.js -s ./init_test.json
...
client_macos_fix_pod:
    export LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8 && pod install --repo-update
